...
METADATA_ANNOT_PATH = "/Users/andrewjones/Documents/beehive/gtex/v8_metadata/GTEx_Analysis_2015-01-12_Annotations_SubjectPhenotypesDD.xlsx"
DATA_DIR = "./out"
...
def main():
    ...
    lvs_shared.index = lv_subject_ids

    n_cc_vars = lvs_shared.shape[1]
    lvs_shared.columns = ["CC{}".format(x+1) for x in np.arange(n_cc_vars)]
    all_data = pd.merge(v8_metadata, lvs_shared, left_on="SUBJID", right_index=True)
    ...
    for ii, curr_metadata_varname in enumerate(metadata_varnames):
        ...
        curr_annot = metadata_col_annots.DOCFILE.values[metadata_col_annots.VARNAME == curr_metadata_varname]

        if curr_metadata_varname == "SEX":
            curr_annot = "Demography"
        elif len(curr_annot) > 0:
            curr_annot = curr_annot[0]
        else:
            curr_annot = "Misc."
    ...
    plt.ylabel("$R^2$")
    plt.xlabel("")
    plt.title("Donor-level metadata")

    plt.tight_layout()
    plt.savefig("./out/metadata_associations_subjects.png")
    plt.show()
